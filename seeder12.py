# seeder12.py — FIRST SEMESTER 2026-2027 OFFICIAL DATASET (FINAL SYNC)
import sys, os
...
SEEDER_LEVEL   = 12
DESCRIPTION    = "Official Database Seeder - 1st Semester 2026-2027 with Strict Mon-Thu 8am-6pm Constraints."
...
# ── FACULTY (Strict Monday-Thursday schedule, 8am-6pm for all; Mary Ann NO Thursday) ────────────
FACULTY_DATA = [
    # Permanent Faculty
    {'name': 'GELERA, ARIES M.',            'status': 'Permanent',  'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    {'name': 'PELIÑA, MARY ANN I.',         'status': 'Permanent',  'days': 'Monday,Tuesday,Wednesday',          'assign_status': 'Announced'},
    {'name': 'ESTONILO, CHRISTOPHER G.',    'status': 'Permanent',  'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    {'name': 'NOCON, YVANA JARDINE R.',     'status': 'Permanent',  'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    {'name': 'NABABLIT, KARLO JOSE E.',     'status': 'Permanent',  'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    {'name': 'VILLANUEVA, LESTER D.',       'status': 'Permanent',  'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    {'name': 'MUYOT, ALLEN JOHN C.',        'status': 'Permanent',  'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    ...
    {'name': 'TOLEDO, IVAN',                'status': 'Part-timer', 'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    {'name': 'ORDOÑA, KARL VINCENT M.',     'status': 'Part-timer', 'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    {'name': 'DCS Teacher G (DON) - CS',    'status': 'Part-timer', 'days': 'Friday', 'assign_status': 'Announced'},
    {'name': 'DCS Teacher H (JM) - IT',     'status': 'Part-timer', 'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'Announced'},
    
    # TBA / Reserved Teachers
    {'name': 'DCS Teacher A (IT)',          'status': 'TBA',        'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'TBA'},
    {'name': 'DCS Teacher B (IT)',          'status': 'TBA',        'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'TBA'},
    {'name': 'DCS Teacher C (CS)',          'status': 'TBA',        'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'TBA'},
    {'name': 'DCS Teacher D (CS)',          'status': 'TBA',        'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'TBA'},
    ...
    {'name': 'T.B.A.',                      'status': 'TBA',        'days': 'Monday,Tuesday,Wednesday,Thursday', 'assign_status': 'TBA'},
]

# ── LOADINGS (FROM NEW WORKLOAD LIST) ─────────────────────────────────────────
...
LOADINGS = [
    ...
    ('AMBIL, KYLE ANGELO', 'ITEC 85', 'Information Assurance and Security I', 2, 1, 2, 3, ['BSIT 301 A', 'BSIT 301 B', 'BSIT 301 C']),
    ...
    ('CABRIDO, ALYANA', 'INSY 55', 'System Analysis and Design', 2, 1, 2, 3, ['BSIT 301 D', 'BSIT 301 E']),
    
    ('CEDILLO, CEDRICK KENN', 'ITEC 80', 'Human Computer Interaction', 2, 1, 2, 3, ['BSCS 401 A', 'BSCS 401 B']),
    ...
    ('MUYOT, ALLEN JOHN C.', 'DCIT 22', 'Computer Programming I', 1, 2, 1, 6, ['BSCS 101 B']),
    ('MUYOT, ALLEN JOHN C.', 'ITEC 200B', 'Capstone Project and Research 2', 3, 0, 3, 0, ['BSIT 401 B', 'BSIT 401 C']),
    ...
    ('PELIÑA, MARY ANN I.', 'DCIT 60', 'Methods of Research', 3, 0, 3, 0, ['BSIT 301 D', 'BSIT 301 E']),
    ('PELIÑA, MARY ANN I.', 'ITEC 200B', 'Capstone Project and Research 2', 3, 0, 3, 0, ['BSIT 401 A']),
    ...
    ('TINAMBACAN, AARON', 'DCIT 26', 'Application Development and Emerging Technologies', 2, 1, 2, 3, ['BSIT 301 D', 'BSIT 301 E']),
    
    ('VILLANUEVA, LESTER D.', 'ITEC 90', 'Network Fundamentals', 2, 1, 2, 3, ['BSIT 301 A', 'BSIT 301 B', 'BSIT 301 C']),
    
    ('DCS Teacher A (IT)', 'COSC 50', 'Discreet Structure', 3, 0, 3, 0, ['BSIT 101 A', 'BSIT 101 B', 'BSIT 101 C']),
    ('DCS Teacher A (IT)', 'ITEC 111', 'Integrated Programming and Technologies 2', 2, 1, 2, 3, ['BSIT 401 A', 'BSIT 401 B', 'BSIT 401 C']),
    ('DCS Teacher A (IT)', 'ITEC 80', 'Human Computer Interaction', 2, 1, 2, 3, ['BSCS 401 C']),
    
    ('DCS Teacher B (IT)', 'COSC 105', 'Intelligence Systems', 2, 1, 2, 3, ['BSCS 401 A', 'BSCS 401 B', 'BSCS 401 C']),
    ('DCS Teacher B (IT)', 'ITEC 116', 'Systems Integration and Architecture 2', 2, 1, 2, 3, ['BSIT 401 A', 'BSIT 401 B', 'BSIT 401 C']),
    
    ('DCS Teacher C (CS)', 'DCIT 26', 'Application Development and Emerging Technologies', 2, 1, 2, 3, ['BSIT 301 A', 'BSIT 301 B', 'BSIT 301 C']),
    ('DCS Teacher C (CS)', 'COSC 111', 'Internet of Things', 2, 1, 2, 3, ['BSCS 401 A', 'BSCS 401 B', 'BSCS 401 C']),
    
    ('DCS Teacher D (CS)', 'DCIT 22', 'Computer Programming I', 1, 2, 1, 6, ['BSCS 101 A']),
    ('DCS Teacher D (CS)', 'DCIT 65', 'Social and Professional Issues', 3, 0, 3, 0, ['BSCS 301 A', 'BSCS 301 B', 'BSCS 301 C', 'BSCS 301 D', 'BSCS 301 E']),
    ('DCS Teacher D (CS)', 'ITEC 85', 'Information Assurance and Security I', 2, 1, 2, 3, ['BSIT 301 D', 'BSIT 301 E']),
    
    ('DCS Teacher E (CS)', 'CvSU 101', 'Institutional Orientation', 1, 0, 1, 0, ['BSIT 101 A', 'BSIT 101 B', 'BSIT 101 C']),
    ('DCS Teacher E (CS)', 'CvSU 101', 'Institutional Orientation', 1, 0, 1, 0, ['BSCS 101 A', 'BSCS 101 B', 'BSCS 101 C']),
    ('DCS Teacher E (CS)', 'ITEC 110', 'System Administration and Maintenance', 2, 1, 2, 3, ['BSIT 401 A', 'BSIT 401 B', 'BSIT 401 C']),
    ('DCS Teacher E (CS)', 'COSC 100', 'Automata Theory and Formal Languages', 3, 0, 3, 0, ['BSCS 401 A', 'BSCS 401 B', 'BSCS 401 C']),
    
    ('DCS Teacher F (CS)', 'COSC 80', 'Operating Systems', 2, 1, 2, 3, ['BSCS 301 A', 'BSCS 301 B', 'BSCS 301 C', 'BSCS 301 D', 'BSCS 301 E']),
    ('DCS Teacher F (CS)', 'COSC 85', 'Networks and Communication', 2, 1, 2, 3, ['BSCS 301 C']),
    
    ('CATALAN, RACQUEL A.', 'ITEC 55', 'Platform Technologies', 2, 1, 2, 3, ['BSIT 201 A', 'BSIT 201 B', 'BSIT 201 C']),
    
    ('TOLEDO, IVAN', 'COSC 55', 'Discreet Structures II', 3, 0, 3, 0, ['BSCS 201 A', 'BSCS 201 B', 'BSCS 201 C', 'BSCS 201 D']),
    
    ('ORDOÑA, KARL VINCENT M.', 'ITEC 90', 'Network Fundamentals', 2, 1, 2, 3, ['BSIT 301 D', 'BSIT 301 E']),
    
    ('DCS Teacher G (DON) - CS', 'COSC 85', 'Networks and Communication', 2, 1, 2, 3, ['BSCS 301 D', 'BSCS 301 E']),
...
def seed_database():
    with app.app_context():
        ...
        for f in FACULTY_DATA:
            # Excepted teachers who stay restricted to Mon-Thu (or Mon-Wed for Mary Ann, or Friday only for DON)
            excepted_names = ["LESTER", "YVANA", "ESTONILO", "MUYOT", "ARIES", "MARY ANN", "OBON", "KARLO", "DON"]
            is_excepted = any(ex in f['name'].upper() for ex in excepted_names)
            
            avail_days = f['days']
            if not is_excepted:
                avail_days = 'Monday,Tuesday,Wednesday,Thursday,Friday,Saturday'
                
            if f["name"].startswith("DCS Teacher"):
                # e.g., 'DCS Teacher G (DON) - CS' -> parts: ['DCS', 'Teacher', 'G', ...]
                parts = f["name"].split()
                letter = parts[2] if len(parts) > 2 else "X"
                emp_id = f'DCS-TCH-{letter}'
            else:
                emp_id = f'DCS-{f["name"][:3].upper()}'
        ...
        for name in sorted(list(all_section_names)):
            ...
            # Monday-Thursday for first years, Monday-Saturday for others
            if yr == 1:
                avail_days = 'Monday,Tuesday,Wednesday,Thursday'
            else:
                avail_days = 'Monday,Tuesday,Wednesday,Thursday,Friday,Saturday'
        ...
        # B. Courses from Full Curriculum
        ...
        # 8. System Settings (Monday to Saturday, 8:00 AM - 6:00 PM)
        db.session.add(SystemSettings(
            start_hour=8, end_hour=18, allowed_days="Monday,Tuesday,Wednesday,Thursday,Friday,Saturday", 
        ...
        # 9. Constraints (MASTER LIST: HC-01 to HC-25, SC-I-01 to SC-II-01)
        CONSTRAINTS = [
            # ── HARD CONSTRAINTS (HC-01 to HC-25) ──────────────────────────────────────
            ...
            {'code': 'FACULTY_DAY_COMPACTION',     'cat': 'Faculty',        'type': 'NC',  'weight': 10,  'name': '(SC-I-04) Faculty Day Compaction', 'desc': 'Balance faculty schedules into dense 2-class-per-day clusters.'},
        ...
        db.session.commit()
        print(f"Seeder {SEEDER_LEVEL} complete! OFFICIAL 1st Semester 2026-2027 Dataset with Strict Mon-Thu Constraints Initialized.")
