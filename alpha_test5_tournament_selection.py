...
class GeneticSchedulerTC5:
    ...
    # ---------------------------------------------------------
    # TOURNAMENT SELECTION & ELITISM (PHASE-AWARE)
    # ---------------------------------------------------------
    ...
    def _soft_phase_select(self, population):
        pop_size = len(population)
        i, j = random.randint(0, pop_size - 1), random.randint(0, pop_size - 1)
        a, b = population[i], population[j]
        if a.soft_score < b.soft_score: return a
        if a.soft_score > b.soft_score: return b
        return a if a.hard_conflicts <= b.hard_conflicts else b
    ...
    def select_parents(self, population, current_phase):
        if current_phase == 'hard':
            ...
            p1 = self._soft_phase_select(population)
            p2 = self._soft_phase_select(population)
        return p1, p2
    ...
    def run_algorithm(self, progress_callback=None):
        start_t0 = time.time()
        print("=== RUNNING ALPHA TEST CASE 5: TOURNAMENT SELECTION & ELITISM ===")
        ...
        for generation in range(1, MAX_GENERATIONS + 1):
            ...
            # Phase-aware elitism sorting
            if hard_phase:
                population.sort(key=lambda x: (x.hard_conflicts, x.soft_score))
            else:
                population.sort(key=lambda x: (x.soft_score, x.hard_conflicts))
...
if __name__ == '__main__':
    print("================================================================")
    print("🧪 ALPHA TESTING: TEST CASE 5 (TOURNAMENT SELECTION & ELITISM)")
    print("================================================================")
    ...
    with app.app_context():
        ...
        scheduler = GeneticSchedulerTC5(courses, sections, faculty, rooms, pre_assignments, {}, start_time=7, end_time=21, allowed_days=active_days)
