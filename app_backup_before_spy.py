...
def render_excel_to_html(ws, variable_map=None, cell_overrides=None,
    ...
    _g_start = None
    _g_end = max_row
    for _r_chk in range(row_start, max_row + 1):
        _row_texts = [str(ws.cell(row=_r_chk, column=_c).value or '').upper() for _c in range(col_start, max_col + 1)]
        _row_t = " ".join(_row_texts)
        if _g_start is None and any(k in _row_t for k in ["MON", "TUE", "WED", "THURS", "FRI", "SAT"]):
            _g_start = _r_chk
        if any(k in _row_t for k in ["PREPARED BY:", "RECOMMENDING APPROVAL:", "APPROVED:"]):
            if _g_start is not None:
                _g_end = _r_chk - 2
                break
    ...
    for r in range(row_start, max_row + 1):
        ...
        for c in range(col_start, max_col + 1):
            ...
            # Diagnostic: Highlight Everything
            if _g_start and _g_end:
                if _g_start <= r <= _g_end:
                    sp.append('background-color: yellow !important')
                else:
                    sp.append('background-color: lightblue !important')
            ...
            _manual_px = _leading_spaces * 10
            _indent_px = int(al.indent) * MDW if al and al.indent else 0
            _total_pad = _indent_px + _manual_px + 4
            
            _is_grid_cell = (_g_start and _g_end and _g_start <= r <= _g_end)

            if cell_overrides and str(cell_overrides.get((r, c), '')).startswith('<'):
                sp.append('padding:0 !important')
            elif not _is_grid_cell:
                # Accurate spacing for signatories/headers (Blue Area)
                sp.append(f'padding:0 4px 0 {_total_pad}px !important')
            else:
                # Standard grid padding (Yellow Area)
                sp.append('padding:0 4px !important')
            if al:
...
def render_excel_to_html_pdf(ws, variable_map=None, cell_overrides=None,
    ...
    for r in range(row_start, max_row + 1):
        ...
        for c in range(col_start, max_col + 1):
            ...
            # INDENTATION: Detect all whitespace
            _leading_spaces = len(raw) - len(raw.lstrip()) if raw else 0
            _space_padding = _leading_spaces * 10
            _indent_px = int(al.indent) * MDW if al and al.indent else 0
            ...
            if al:
                ...
                if not is_grid_row:
                    # Non-grid: Forced overflow (no-wrap)
                    sp.append('white-space:nowrap')
                else:
